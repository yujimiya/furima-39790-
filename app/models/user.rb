class User < ApplicationRecord
  ...
  validates :password, presence: true, format: { with: /\A(?=.[a-z])(?=.*?[\d])[a-z\d]+\z/i, messaege: 'は、英数字混合である実用があります'}
  validates :last_name, presence: true, format: { with: /\A[ぁ-んァ-ヶ一-龥々ー]+\z/, message: "は、全角ひらがな、全角カタカナ、漢字で入力して下さい" }
  validates :first_name, presence: true, format: { with: /\A[ぁ-んァ-ン一-龥々]+\z/, message: "は、全角ひらがな、全角カタカナ、漢字で入力して下さい" }
